...
export function shipmentReferences(data,shipment){const transfer=(data.transfers||[]).find(t=>t.id===shipment.transferId),order=shipmentOrder(data,shipment);return [shipment.code,shipment.id,shipment.transferId,shipment.orderId,shipment.documentNumber,shipment.orderNumber,transfer?.documentNumber,transfer?.orderNumber,transfer?.externalNumber,order?.id,order?.externalNumber,order?.finalDocumentNumber].filter(Boolean);}
export function shipmentSearchText(shipment,data={}){return [...shipmentReferences(data,shipment),shipment.sourceSiteId,shipment.destinationSiteId,shipment.destinationName].filter(Boolean).map(searchable).join(' ');}
export function shipmentMatches(shipment,value,data={}){const query=searchable(value);if(!query)return true;return shipmentSearchText(shipment,data).split(' ').some(candidate=>candidate===query||candidate.includes(query)||query.includes(candidate));}
export function shipmentByCode(data,value){const query=searchable(value);if(!query)return null;const shipments=data.shipments||[];return shipments.find(s=>shipmentReferences(data,s).some(candidate=>searchable(candidate)===query))||shipments.find(s=>shipmentMatches(s,query,data));}
...
export function createShipment(data,transfer,{driverName='',packageCount=1,notes='',userId=data.session?.userId,at=now()}={}){
  ...
  const shipment={id,code,transferId:transfer.id,orderId:transfer.orderId||null,documentNumber:transfer.documentNumber||null,orderNumber:transfer.orderNumber||transfer.externalNumber||null,sourceSiteId:transfer.sourceSiteId,destinationSiteId:transfer.destinationSiteId||null,destinationType:transfer.destinationType||'SITE',destinationName:transfer.destinationName||'',destinationAddress:transfer.destinationAddress||'',destinationContact:transfer.destinationContact||'',status:'LISTA_RETIRO',driverName:driverName||transfer.driver||'',transporterUserId:null,packageCount:Math.max(1,n(packageCount)),containerId:`PAL-${code}`,notes,items:(transfer.items||[]).map(i=>({code:i.code||i.productCode,expectedQty:n(i.qty),receivedQty:null})).filter(i=>i.code&&i.expectedQty>0),createdAt:at,createdBy:userId,sealedAt:at,sealedBy:userId,events:[{at,userId,message:'Carga sellada y lista para retiro'}]};
