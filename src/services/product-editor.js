...
function dialogHtml(){
  return `<dialog id="product-editor-dialog" class="product-editor-dialog"><form id="product-editor-form" class="product-editor-card">
    ...
    <section class="product-editor-section"><div class="section-mini-head"><div><b>Códigos asociados</b><small>Un solo producto maestro puede responder a SKU, código de importación, tienda, Shopify u otros códigos.</small></div></div>
      ...
      <div class="product-code-add"><label>Origen / canal<select id="pe-code-type"><option value="EASY">Easy</option><option value="SODIMAC">Sodimac</option><option value="CONSTRUMART">Construmart</option><option value="CAJA">Código de caja</option><option value="KAME">Kame</option><option value="SHOPIFY">Shopify / web</option><option value="PROVEEDOR">Proveedor</option><option value="OTRO">Otro</option></select></label><label>Código<div class="entrada-con-camara"><input id="pe-alt-code" autocomplete="off" placeholder="Escanea o escribe el código"><button id="pe-alt-code-camera" class="scan-button" type="button" title="Escanear código asociado">▣</button></div></label><label>Presentación<select id="pe-code-presentation"><option value="UNIDAD">Unidad</option><option value="CAJA">Caja</option><option value="PACK">Pack</option><option value="BULTO">Bulto</option></select></label><label>Unidades<input id="pe-code-units" type="number" min="1" step="1" placeholder="Ej. 25" required></label><label>Excel cantidad<select id="pe-code-excel-mode"><option value="UNITS">Unidades</option><option value="PACKS">Cajas / packs</option></select></label><label>Etiqueta opcional<input id="pe-code-label" placeholder="Ej. SKU Vitacura"></label><button type="button" id="pe-add-code" class="secondary">+ Asociar código</button></div>
    </section>
...
function fill(code){
  ...
  const codes=document.querySelector('#pe-codes-list'),isSuperuser=['ADMIN_GLOBAL','ADMINISTRADOR'].includes(currentUser()?.role),historical=[...new Set((p.previousCodes||[]).map(normalizeProductCode).filter(x=>x&&x!==normalizeProductCode(p.code)))],alt=(store.data.product_codes||[]).filter(x=>x.productId===p.id&&x.active!==false); codes.innerHTML=`<div class="product-code-row primary-code"><div><b>${esc(p.code)}</b><small>Código principal</small></div><span class="badge ok">PRINCIPAL</span></div>${isSuperuser&&historical.length?`<div class="product-code-row historical-code"><div><b>SKU históricos</b><small>Eliminar todos los códigos anteriores que todavía responden en búsquedas.</small></div><button type="button" class="danger-action small purge-all-historical-codes">Eliminar todos</button></div>`:''}`+historical.map(code=>`<div class="product-code-row historical-code" data-code="${esc(code)}"><div><b>${esc(code)}</b><small>SKU histórico · todavía responde en la búsqueda</small></div>${isSuperuser?`<button type="button" class="danger-action small purge-historical-code" data-code="${esc(code)}">Eliminar SKU</button>`:'<span class="badge neutral">HISTÓRICO</span>'}</div>`).join('')+alt.map(x=>`<div class="product-code-row editable-alt-code" data-id="${esc(x.id)}"><label>Código<input class="alt-code-value" value="${esc(x.code)}" autocomplete="off"></label><label>Origen<select class="alt-code-type">${['EASY','SODIMAC','CONSTRUMART','CAJA','KAME','SHOPIFY','PROVEEDOR','SKU','IMPORTACION','TIENDA','CONTROL','OTRO'].map(t=>`<option value="${t}" ${t===(x.source||x.type||'OTRO')?'selected':''}>${t}</option>`).join('')}</select></label><label>Presentación<select class="alt-code-presentation">${['UNIDAD','CAJA','PACK','BULTO','OTRO'].map(t=>`<option value="${t}" ${t===(x.presentation||'UNIDAD')?'selected':''}>${t}</option>`).join('')}</select></label><label>Unidades<input class="alt-code-units" type="number" min="1" step="1" value="${Math.max(1,Number(x.unitsPerPresentation||1))}"></label><label>Excel<select class="alt-code-excel-mode"><option value="UNITS" ${x.excelQtyMode!=='PACKS'?'selected':''}>Unidades</option><option value="PACKS" ${x.excelQtyMode==='PACKS'?'selected':''}>Cajas / packs</option></select></label><label>Etiqueta<input class="alt-code-label" value="${esc(x.label||'')}" placeholder="Opcional"></label><div class="alt-code-actions"><button type="button" class="secondary small save-alt-code" data-id="${esc(x.id)}">Guardar código</button><button type="button" class="ghost small remove-alt-code" data-id="${esc(x.id)}">Quitar</button></div></div>`).join('');
...
export function openProductEditor(code,{onSaved}={}){
  ...
  ['pe-code-type','pe-alt-code','pe-code-presentation','pe-code-units','pe-code-excel-mode','pe-code-label'].forEach(id=>{const el=document.querySelector(`#${id}`);if(el)el.disabled=!permission.associate;});
  ...
  document.querySelectorAll('.purge-historical-code,.purge-all-historical-codes').forEach(b=>b.disabled=!['ADMIN_GLOBAL','ADMINISTRADOR'].includes(currentUser()?.role));
  ...
  document.querySelector('#pe-add-code').onclick=async()=>{if(!permission.associate)return;const code=normalizeProductCode(document.querySelector('#pe-alt-code').value),unitsInput=document.querySelector('#pe-code-units'),unitsRaw=unitsInput.value.trim(),units=Number(unitsRaw);if(!code){toast('Escribe el código a asociar');return;}if(!unitsRaw||!Number.isInteger(units)||units<1){unitsInput.focus();toast('Indica cuántas unidades contiene este código o empaque');return;}if(codeInUse(code,p.id)){toast('Ese código ya pertenece a otro producto');return;}try{await store.commit(st=>addProductCode(st,p.id,code,document.querySelector('#pe-code-type').value,document.querySelector('#pe-code-label').value,{source:document.querySelector('#pe-code-type').value,presentation:document.querySelector('#pe-code-presentation').value,unitsPerPresentation:units,excelQtyMode:document.querySelector('#pe-code-excel-mode').value}),`Código ${code} asociado a ${p.code}`,{operations:['codesAssociate']});close();openProductEditor(p.code,{onSaved});toast('Código asociado');}catch(err){toast(err.message);}};
  document.querySelectorAll('.save-alt-code').forEach(b=>b.onclick=async()=>{if(!permission.associate)return;const row=b.closest('.editable-alt-code'),x=(store.data.product_codes||[]).find(c=>c.id===b.dataset.id);if(!x)return;const next=normalizeProductCode(row.querySelector('.alt-code-value').value),type=row.querySelector('.alt-code-type').value,label=row.querySelector('.alt-code-label').value.trim(),presentation=row.querySelector('.alt-code-presentation').value,units=Math.max(1,Number(row.querySelector('.alt-code-units').value||1)),excelQtyMode=row.querySelector('.alt-code-excel-mode').value;if(!next){toast('El código no puede quedar vacío');return;}if(codeInUse(next,p.id)&&next!==normalizeProductCode(x.code)){toast('Ese código ya pertenece a otro producto');return;}await store.commit(st=>{const c=(st.product_codes||[]).find(v=>v.id===b.dataset.id);if(c){c.code=next;c.type=type;c.source=type;c.label=label;c.presentation=presentation;c.unitsPerPresentation=units;c.excelQtyMode=excelQtyMode;c.active=true;}},`Código asociado ${x.code} → ${next} actualizado en ${p.code}`,{operations:['codesAssociate']});close();openProductEditor(p.code,{onSaved});toast('Código asociado actualizado');});
  ...
  const purgeHistorical=async stale=>{if(!['ADMIN_GLOBAL','ADMINISTRADOR'].includes(currentUser()?.role))return;stale=normalizeProductCode(stale);if(!stale||stale===normalizeProductCode(p.code))return;const ok=await requireAdminSupercode(`Vas a eliminar el SKU histórico ${stale} de ${p.code}. Dejará de identificar este producto en búsquedas y escaneos. No se eliminará el producto, su stock ni su historial de movimientos.`,{title:'Eliminar SKU histórico',buttonLabel:'Eliminar SKU'});if(!ok)return;try{const at=new Date().toISOString();await store.commit(st=>{const pp=(st.products||[]).find(x=>x.id===p.id);if(!pp)throw new Error('El producto ya no existe.');pp.previousCodes=(pp.previousCodes||[]).filter(code=>normalizeProductCode(code)!==stale);st.audit=st.audit||[];st.audit.unshift({id:`AUD-SKU-PURGE-${Date.now()}`,type:'SKU_HISTORICAL_REMOVED',message:`SKU histórico ${stale} eliminado de ${pp.code}. Ya no responde en búsquedas ni escaneos.`,userId:st.session.userId,at});},`SKU histórico ${stale} eliminado de ${p.code}`,{operations:['productsEdit']});await store.reload({emit:false});close();onSaved?.(p.code);openProductEditor(p.code,{onSaved});await notice('SKU eliminado',`${stale} dejó de estar asociado a ${p.code}. El producto, stock y movimientos permanecen intactos.`,'success');}catch(err){await notice('No se pudo eliminar el SKU',err?.message||'La eliminación no pudo guardarse en el servidor. No se modificó el producto.','error');}};
  document.querySelectorAll('.purge-historical-code').forEach(b=>b.onclick=()=>purgeHistorical(b.dataset.code));
  const purgeAll=document.querySelector('.purge-all-historical-codes');if(purgeAll)purgeAll.onclick=async()=>{if(!['ADMIN_GLOBAL','ADMINISTRADOR'].includes(currentUser()?.role))return;const staleCodes=[...new Set((p.previousCodes||[]).map(normalizeProductCode).filter(code=>code&&code!==normalizeProductCode(p.code)))];if(!staleCodes.length){toast('Este producto no tiene SKU históricos para eliminar');return;}const ok=await requireAdminSupercode(`Vas a eliminar TODOS los SKU históricos (${staleCodes.length}) de ${p.code}: ${staleCodes.join(', ')}. Ninguno volverá a identificar este producto en búsquedas o escaneos.`,{title:'Eliminar todos los SKU históricos',buttonLabel:'Eliminar todos'});if(!ok)return;try{const at=new Date().toISOString();await store.commit(st=>{const pp=(st.products||[]).find(x=>x.id===p.id);if(!pp)throw new Error('El producto ya no existe.');pp.previousCodes=[];st.audit=st.audit||[];st.audit.unshift({id:`AUD-SKU-PURGE-ALL-${Date.now()}`,type:'SKU_HISTORICAL_ALL_REMOVED',message:`Se eliminaron ${staleCodes.length} SKU históricos de ${pp.code}: ${staleCodes.join(', ')}.`,userId:st.session.userId,at});},`Todos los SKU históricos eliminados de ${p.code}`,{operations:['productsEdit']});await store.reload({emit:false});close();onSaved?.(p.code);openProductEditor(p.code,{onSaved});await notice('SKU históricos eliminados',`${staleCodes.length} código(s) anteriores fueron eliminados. Solo permanece el SKU principal ${p.code} y los códigos asociados activos.`,'success');}catch(err){await notice('No se pudieron eliminar los SKU',err?.message||'La eliminación no pudo guardarse en el servidor. No se modificó el producto.','error');}};
  const foundStockButton=document.querySelector('#pe-found-stock');foundStockButton.hidden=!permissions().physicalStock;foundStockButton.onclick=()=>{const current=document.querySelector('#pe-original-code').value;dlg.close();openPhysicalStockEntry(current,{onSaved:newCode=>openProductEditor(newCode,{onSaved})});};
  ...
  document.querySelector('#product-editor-form').onsubmit=async e=>{
    ...
    await store.commit(s=>{
      ...
      qtyChanges.forEach(ch=>{const inv=s.inventory.find(i=>i.id===ch.id);if(!inv)return;inv.qty=ch.after;s.movements.unshift({id:`MOV-AJ-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,siteId:activeSiteId(s),type:'AJUSTE_INVENTARIO',productCode:newCode,qty:Math.abs(ch.after-ch.before),delta:ch.after-ch.before,beforeQty:ch.before,afterQty:ch.after,from:ch.locationId,to:ch.locationId,palletId:ch.palletId,reason:`${reason} · Conteo ${ch.before} → ${ch.after} (${deltaText(ch.after-ch.before)})`,userId:s.session.userId,at});});
