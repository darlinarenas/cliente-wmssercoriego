...
export function openProductEditor(code,{onSaved}={}){
  ...
  document.querySelectorAll('.purge-historical-code').forEach(b=>b.disabled=currentUser()?.role!=='ADMIN_GLOBAL');
  ...
  document.querySelectorAll('.purge-historical-code').forEach(b=>b.onclick=async()=>{if(currentUser()?.role!=='ADMIN_GLOBAL')return;const stale=normalizeProductCode(b.dataset.code);if(!stale||stale===normalizeProductCode(p.code))return;const ok=await requireAdminSupercode(`Vas a eliminar el SKU histórico ${stale} de ${p.code}. Dejará de identificar este producto en búsquedas y escaneos. No se eliminará el producto, su stock ni su historial de movimientos.`,{title:'Eliminar SKU histórico',buttonLabel:'Eliminar SKU'});if(!ok)return;const at=new Date().toISOString();await store.commit(st=>{const pp=(st.products||[]).find(x=>x.id===p.id);if(!pp)return;pp.previousCodes=(pp.previousCodes||[]).filter(code=>normalizeProductCode(code)!==stale);st.audit=st.audit||[];st.audit.unshift({id:`AUD-SKU-PURGE-${Date.now()}`,type:'SKU_HISTORICAL_REMOVED',message:`SKU histórico ${stale} eliminado de ${pp.code}. Ya no responde en búsquedas ni escaneos.`,userId:st.session.userId,at});},`SKU histórico ${stale} eliminado de ${p.code}`,{operations:['productsEdit']});close();openProductEditor(p.code,{onSaved});await notice('SKU eliminado',`${stale} dejó de estar asociado a ${p.code}. El producto, stock y movimientos permanecen intactos.`,'success');});
