...
export function openProductEditor(code,{onSaved}={}){
  ...
  document.querySelector('#pe-add-code').onclick=async()=>{if(!permission.associate)return;const code=normalizeProductCode(document.querySelector('#pe-alt-code').value);if(!code){toast('Escribe el código a asociar');return;}if(codeInUse(code,p.id)){toast('Ese código ya pertenece a otro producto');return;}try{await store.commit(st=>addProductCode(st,p.id,code,document.querySelector('#pe-code-type').value,document.querySelector('#pe-code-label').value,{source:document.querySelector('#pe-code-type').value,presentation:document.querySelector('#pe-code-presentation').value,unitsPerPresentation:Number(document.querySelector('#pe-code-units').value||1),excelQtyMode:document.querySelector('#pe-code-excel-mode').value}),`Código ${code} asociado a ${p.code}`,{operations:['codesAssociate']});close();openProductEditor(p.code,{onSaved});toast('Código asociado');}catch(err){toast(err.message);}};
