...
export function renderLabelImage(data,{type,w,h,dpi=203,brand=false},makeCanvas=()=>document.createElement('canvas')){
 ...
 let fields=physical?[{text:data.company,size:small,lines:1},{text:title,size:big,lines:1}]:type==='PRODUCTO'?[{text:code,size:big*1.22,lines:1},{text:title,size:big*.82,lines:2}]:type==='PALLET'?[{text:data.company,size:small,lines:1},{text:'PALLET',size:small,lines:1},{text:title,size:big,lines:2}]:[{text:data.company,size:small,lines:1},{text:title,size:big,lines:3},...(data.lines||[]).filter(Boolean).slice(0,2).map(text=>({text,size:small,lines:1}))];
 ...
 let y=Math.round((H-total)/2);if(physical)y=Math.max(margin,y-Math.round(.9*dpmm));const top=y;
