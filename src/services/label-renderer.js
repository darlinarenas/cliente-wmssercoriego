...
export function renderLabelImage(data,{type,w,h,dpi=203,brand=false},makeCanvas=()=>document.createElement('canvas')){
 ...
 const W=canvas.width,H=canvas.height,margin=Math.round(Math.min(4,w*.06,h*.12)*dpmm),usable=W-2*margin,available=H-2*margin;
 ...
 const gap=Math.round((h<=30?.7:1.3)*dpmm),compact=w<=50||h<=30;
 const small=(compact?2.3:2.8)*dpmm,big=(compact?4:5.5)*dpmm;
 const physical=['UBICACION','RACK'].includes(type),title=type==='UBICACION'?code:(data.title||code);
 ...
 for(let scale=1;scale>=.55;scale-=.05){try{layout=fields.map(f=>fit(f,scale));caption=fit({text:code,size:small,lines:1},scale);footer=brand?fit({text:'By Vexhora',size:small*.85,lines:1},scale):null;const texts=layout.reduce((n,f)=>n+f.height,0)+caption.height+(footer?.height||0),gaps=gap*(layout.length+1+(footer?1:0));barH=Math.min(Math.round(available*.36),available-texts-gaps);total=texts+gaps+barH;if(barH>=Math.min(6,h*.22)*dpmm){lastError=null;break;}lastError=new Error(`La altura de ${code} es insuficiente para texto, barras y leyenda. Aumenta el alto.`);}catch(e){lastError=e;}}
