...
function targetLineForSurplus(session,code){const counted=(session.lines||[]).filter(l=>String(l.productCode)===String(code)&&Number(l.qty)>0&&l.locationId).sort((a,b)=>Number(b.qty||0)-Number(a.qty||0))[0];if(counted)return counted;const placement=session.reviewCorrectionPlacementsByProduct?.[String(code)];return placement?.locationId?{productCode:code,locationId:placement.locationId,palletId:placement.palletId||null,rackId:placement.rackId||null,qty:Number(session.reviewOverridesByProduct?.[String(code)]||0),reviewCorrection:true}:null;}
