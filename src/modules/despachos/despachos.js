...
export function renderTransfers(root){
 ...
 if(!t){
   ...
   document.querySelector('#create-transfer').onclick=async(e)=>{e.preventDefault();const idx=d.transfers.length+1,id=`TRF-${String(idx).padStart(6,'0')}`,dest=document.querySelector('#tr-dest').value,driver=document.querySelector('#tr-driver').value.trim(),destinationName=d.sites.find(s=>s.id===dest)?.name||dest,now=new Date().toISOString();if(!dest){toast('No hay otro centro disponible en esta empresa');return;}await store.commit(s=>s.transfers.unshift({id,sourceSiteId:activeSiteId(s),destinationSiteId:dest,destinationName,driver,status:'PREPARANDO',createdAt:now,items:[],supervisedBy:null,dispatchedBy:null}),`Salida ${id} creada`);dlg.close();toast('Salida creada');renderTransfers(root);};
 }else{
