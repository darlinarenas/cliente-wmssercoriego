...
function previewHtml(){
  ...
  const invalidRows=invalid?`<details class="recon-invalid-details" open><summary>Ver fila(s) inválida(s) <b>${invalid}</b></summary><div class="recon-invalid-list">${preview.invalid.map(x=>`<div><b>Fila ${x.row}</b><span>${esc(x.code||'Sin código')}</span><strong>${esc(x.reason)}</strong></div>`).join('')}</div></details>`:'';
  const duplicateRows=duplicates?`<details class="recon-invalid-details"><summary>Ver duplicados <b>${duplicates}</b></summary><div class="recon-invalid-list">${preview.duplicates.map(x=>`<div><b>Fila ${x.row}</b><span>${esc(x.code)}</span><strong>Repite fila ${x.firstRow}</strong></div>`).join('')}</div></details>`:'';
  ...
  return `<div class="recon-preflight"><div class="recon-preflight-grid"><div><small>Filas procesadas</small><b>${total}</b></div><div class="ok"><small>Reconocidos</small><b>${recognized}</b></div><div class="${missing?'warn':'ok'}"><small>Nuevos / faltantes</small><b>${missing}</b></div><div class="${duplicates?'warn':''}"><small>Duplicados</small><b>${duplicates}</b></div><div class="${invalid?'danger':''}"><small>Inválidos</small><b>${invalid}</b></div></div>${status}${invalidRows}${duplicateRows}${missingRows}</div>`;
}
