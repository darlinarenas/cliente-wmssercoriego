...
let preview=null;
const WMS_TEMPLATE_ROWS=[
  ['CODIGO','STOCK','DESCRIPCION'],
  ['706420A',150,'Ejemplo: producto del ERP'],
  ['706310A',2440,'Ejemplo: segundo producto']
];
function downloadWmsTemplate(){
  const escXml=value=>String(value??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  const rows=WMS_TEMPLATE_ROWS.map((row,r)=>`<Row>${row.map((value,c)=>`<Cell${r>0&&c===1?' ss:StyleID="Number"':''}><Data ss:Type="${r>0&&c===1?'Number':'String'}">${escXml(value)}</Data></Cell>`).join('')}</Row>`).join('');
  const xml=`<?xml version="1.0"?>
<?mso-application progid="Excel.Sheet"?>
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
<Styles><Style ss:ID="Header"><Font ss:Bold="1"/><Interior ss:Color="#EAF0F8" ss:Pattern="Solid"/></Style><Style ss:ID="Number"><NumberFormat ss:Format="0"/></Style></Styles>
<Worksheet ss:Name="CARGA_WMS"><Table>${rows}</Table></Worksheet>
<Worksheet ss:Name="INSTRUCCIONES"><Table>
<Row><Cell ss:StyleID="Header"><Data ss:Type="String">CAMPO</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">USO</Data></Cell></Row>
<Row><Cell><Data ss:Type="String">CODIGO</Data></Cell><Cell><Data ss:Type="String">Obligatorio. SKU principal o código ya asociado en el WMS.</Data></Cell></Row>
<Row><Cell><Data ss:Type="String">STOCK</Data></Cell><Cell><Data ss:Type="String">Obligatorio. Cantidad total del producto en el centro seleccionado.</Data></Cell></Row>
<Row><Cell><Data ss:Type="String">DESCRIPCION</Data></Cell><Cell><Data ss:Type="String">Opcional. Referencia humana; no identifica el producto.</Data></Cell></Row>
<Row><Cell><Data ss:Type="String">IMPORTANTE</Data></Cell><Cell><Data ss:Type="String">No cambies CODIGO ni STOCK. Un SKU omitido no se modifica; STOCK 0 deja ese SKU en cero.</Data></Cell></Row>
  ...
  a.href=url;a.download='PLANTILLA-UNIVERSAL-CARGA-STOCK-WMS.xls';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
...
export function parseRows(rows){
  ...
  const h=rows[0].map(normalizeHeader),ci=h.findIndex(x=>['CODIGO','SKU','CODIGO_PRODUCTO'].includes(x)),si=h.findIndex(x=>['STOCK','CANTIDAD','EXISTENCIA','EXISTENCIAS','SALDO'].includes(x));
  if(ci<0||si<0)return {errors:['El Excel debe incluir CODIGO (o SKU) y STOCK (también acepta CANTIDAD, EXISTENCIA o SALDO).'],items:[]};
...
export function renderReconciliation(root){
  ...
  document.querySelector('#download-wms-template').onclick=downloadWmsTemplate;
  document.querySelector('#recon-file').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{preview=parseRows(await parseXlsx(file));preview.fileName=file.name;renderReconciliation(root);}catch(err){preview={errors:[err.message],items:[],fileName:file.name};renderReconciliation(root);}};
