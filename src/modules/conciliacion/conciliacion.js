...
export function parseRows(rows){
  ...
  rows.slice(1).forEach((r,n)=>{
    const raw=String(r[ci]??'').trim(),normalized=normalizeProductCode(raw),qtyRaw=r[si],qty=typeof qtyRaw==='number'?qtyRaw:Number(String(qtyRaw??'').trim().replaceAll(',','')),description=di>=0?String(r[di]??'').trim():'';
