...
export function renderReconciliation(root){
  ...
  document.querySelector('#recon-file').onchange=async e=>{try{preview=parseRows(await parseXlsx(e.target.files?.[0]));renderReconciliation(root);}catch(err){preview={errors:[err.message],items:[]};renderReconciliation(root);}};
