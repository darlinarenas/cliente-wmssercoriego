...
function tempOpts(){const site=activeSiteId();return store.data.locations.filter(l=>l.siteId===site&&l.active&&l.kind==='POR_UBICAR').map(l=>`<option value="${esc(l.id)}">${esc(l.label||l.id)} · ${esc(l.id)} · ${esc(l.status)}</option>`).join('');}
...
function receiptPanel(r){
 ...
 <div class="receipt-close receipt-close-grid"><label>Ubicación temporal<select id="rec-temp" required><option value="">Seleccionar…</option>${tempOpts()}</select><button id="open-temp-location" class="ghost small temp-location-shortcut" type="button">+ Crear zona temporal</button></label><label>Supervisado / revisado por<select id="rec-supervisor" required><option value="">Seleccionar…</option>${userOpts()}</select></label><button id="close-receipt" class="primary">Cerrar recepción → POR UBICAR</button></div></section>`;
}
...
export function renderReceipts(root){
 ...
 <dialog id="temp-location-dialog"><form id="temp-location-form"><div class="dialog-head"><div><h3>Nueva zona temporal</h3><small>Solo para el centro activo</small></div><button type="button" id="cancel-temp-location-x" class="ghost">×</button></div><label>Código de ubicación<input id="temp-location-code" required maxlength="40"></label><label>Nombre / referencia física<input id="temp-location-label" required maxlength="80" placeholder="Ej. Recepción junto a puerta 2"></label><div class="dialog-actions"><button type="button" id="cancel-temp-location" class="ghost">Cancelar</button><button class="primary" type="submit">Crear zona</button></div></form></dialog>
 <dialog id="new-receipt-dialog"><form id="new-receipt-form"><div class="dialog-head"><h3>Nueva recepción</h3><button type="button" id="cancel-new-receipt-x" class="ghost">×</button></div><label>Identificación del palet<input id="new-pallet-id" required maxlength="40"><small>Editable. Debe ser único en todo el WMS.</small></label><label>Origen<select id="new-origin"><option>Importación</option><option>Proveedor</option><option>Tienda / devolución</option><option>Cliente / devolución</option><option>Otro</option></select></label><label>Quién trae<input id="new-brought" placeholder="Nombre conductor / transportista"></label><label>Observación<input id="new-note" placeholder="Opcional"></label><div class="dialog-actions"><button type="button" id="cancel-new-receipt" class="ghost">Cancelar</button><button id="create-receipt" class="primary" type="submit">Crear y comenzar</button></div></form></dialog>`,'recepciones');
 wireShell();
