...
function trazaProducto(code){
  ...
  return `<section class="panel trace-card"><div class="panel-head"><div><span class="eyebrow">FICHA DE TRAZABILIDAD</span><h3>${esc(canonical)} · ${esc(product.name)}</h3><small>${esc(product.description||'Descripción no registrada')} · ${esc(product.family||'Sin clasificar')} · Total localizado actual: ${total}${(product.previousCodes||[]).length?` · Código(s) anterior(es): ${esc(product.previousCodes.join(', '))}`:''}</small></div>${badge(`${timeline.length} eventos`,'neutral')}</div>
...
export function renderHistory(root){
 ...
 <section class="panel history-search-panel"><label>Buscar absolutamente en todo el historial<div class="history-search"><span>⌕</span><input id="historial-search" placeholder="Ej.: 448660, PAL-0101, Importación, Darlin, REC-PU-01…" autocomplete="off"><button id="clear-history" class="ghost small" type="button">Limpiar</button></div></label><div class="history-search-meta"><small id="historial-result-count">Mostrando actividad reciente</small><span>Si escribes un código exacto, verás su ficha completa de trazabilidad.</span></div></section>
