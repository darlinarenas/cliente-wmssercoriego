...
function movementOperator(){const u=store.data.users.find(x=>x.id===store.data.session.userId);return ['OPERADOR_BODEGA','OPERADOR_RECEPCION'].includes(effectiveRole(u,activeSiteId()));}
...
export function renderMovements(root){
 ...
 root.innerHTML=shell('Mover / Reubicar',`<div class="page-intro ${movementOperator()?'operator-page-intro':''}"><div><span class="eyebrow">MOVIMIENTO CONTROLADO</span><h2>Mover sin perder trazabilidad</h2><p>La cantidad se descuenta matemáticamente del origen y se suma al destino. El saldo restante queda visible en su ubicación anterior.</p></div></div>
 <form id="move-form" class="panel form-panel ${movementOperator()?'operator-movement-compact':''}"><div class="form-grid"><label>Producto<div class="mv-product-search"><div class="entrada-con-camara"><input id="mv-product-search" autocomplete="off" placeholder="Escanea o busca por código / nombre" aria-label="Buscar producto"><button id="camara-mv-product" class="scan-button" type="button" title="Escanear código con cámara" aria-label="Escanear código con cámara">▣</button></div><input id="mv-product" type="hidden"><div id="mv-product-results" class="mv-product-results" hidden></div><small>Escribe el código completo o parcial, el nombre del producto o escanéalo con la cámara.</small></div></label><label>Cantidad<input id="mv-qty" type="number" min="1" required></label></div>
 ${movementOperator()?`<details class="operator-extra movement-stock-help"><summary>Ver stock y ubicaciones del producto</summary><div id="mv-stock-preview" class="stock-preview">${stockBox('')}</div></details>`:`<div id="mv-stock-preview" class="stock-preview">${stockBox('')}</div>`}
 ...
 ${movementOperator()?'<details class="operator-extra movement-reason-help"><summary>Motivo / opciones adicionales</summary>':''}<label>Motivo<select id="mv-reason"><option>Organizar recepción</option><option>Reorganización</option><option>Reposición</option><option>Consolidación</option><option>Mejor acceso</option><option>Liberar espacio</option><option>Error de ubicación</option><option>Cambio de layout</option></select></label>${movementOperator()?'</details>':''}<button class="primary operator-main-action" type="submit">Confirmar movimiento</button></form>${mapaDestinoMovimientoHtml()}`,'movimientos');wireShell();
