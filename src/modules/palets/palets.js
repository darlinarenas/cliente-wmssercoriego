...
function detallePalet(p){
  ...
  return `<section class="panel pallet-detail-panel">
    <div class="panel-head pallet-detail-head"><div><span class="eyebrow">PALLET FÍSICO · UNIDAD COMPLETA</span><h2>${esc(nombrePalet(p))}</h2><small>${esc(String(p.status||'').replaceAll('_',' '))} · ${esc(p.locationId||'Sin ubicación')}</small></div><div class="pallet-head-actions"><a class="ghost" href="#/palets">Cerrar ×</a></div></div>
