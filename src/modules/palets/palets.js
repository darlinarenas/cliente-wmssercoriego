...
function detallePalet(p){
  ...
  if(p.sourceShipmentId&&p.status==='UBICADO')return `<section class="panel pallet-detail-panel"><div class="panel-head pallet-detail-head"><div><span class="eyebrow">TAREA COMPLETADA</span><h2>${esc(p.id)}</h2><small>UBICADO · ${esc(p.locationId||'Sin ubicación')}</small></div><a class="ghost" href="#/palets">Cerrar ×</a></div><div class="success-box"><b>✓ Pallet ubicado correctamente</b><span>La tarea de ubicación fue cerrada y ya no puede ejecutarse nuevamente desde este flujo.</span><small>Para una reubicación posterior utiliza el módulo Mover / reubicar, que conservará la trazabilidad.</small></div><div class="pallet-meta-grid"><span><b>Ubicación final</b><small>${esc(p.locationId)}</small></span><span><b>Contenido</b><small>${agrupado.length} productos · ${totalUnidades(p.id)} unidades</small></span></div></section>`;
...
export function renderPallets(root){
  ...
  root.innerHTML=shell('Palets',body,'palets');wireShell();if(p&&!(p.sourceShipmentId&&p.status==='UBICADO'))wireDetail(p.id);else if(!p)document.querySelectorAll('.pallet-card-click').forEach(b=>b.onclick=()=>location.hash=`#/palets?id=${encodeURIComponent(b.dataset.pallet)}`);
}
