...
import { addProductCode,normalizeProductCode,productAliases,resolveProduct,resolveProductCode } from '../../services/product-codes.js';
...
function productCard(product,scanned,allowed){
  const sites=stockSitesOrdered(product.code),active=sites.find(s=>s.siteId===activeSiteId()),aliases=productAliases(product),matched=resolveProductCode(scanned);
  const packagingAction=allowed.associate&&matched?.record?'<button id="code-configure-packaging" class="secondary" type="button">Configurar empaque</button>':'';
  return `<section class="panel code-result-card found compact-code-card"><div class="code-result-head"><div><span class="eyebrow">CÓDIGO ENCONTRADO</span><h2>${esc(product.name||'Producto sin nombre')}</h2><p><b>${esc(scanned)}</b> → producto maestro <b>${esc(product.code)}</b>.</p>${matched?.record?`<small>${esc(matched.source)} · ${esc(matched.presentation)} × ${matched.unitsPerPresentation} · Excel ${matched.excelQtyMode==='PACKS'?'en packs':'en unidades'}</small>`:''}</div><span class="badge ok">REGISTRADO</span></div><div class="code-result-metrics"><span><small>Centro activo</small><b>${Number(active?.qty||0)} un.</b></span><span><small>Stock global</small><b>${totalCompanyStock(product.code)} un.</b></span><span><small>Códigos</small><b>${aliases.length}</b></span></div><details class="code-compact-details"><summary>Códigos reconocidos <b>${aliases.length}</b></summary><div class="code-alias-list">${aliases.map(code=>`<span class="${code===normalizeProductCode(scanned)?'active':''}">${esc(code)}</span>`).join('')}</div></details>${allowed.associate?'':`<div class="info-box"><b>Modo consulta.</b> Tu rol puede identificar productos, pero las asociaciones y correcciones requieren un Encargado o Administrador.</div>`}<div class="code-primary-actions">${allowed.associate?'<button id="code-associate-another" class="primary" type="button">▣ Asociar código</button>':''}${packagingAction}${allowed.associate?'<button id="code-manage-codes" class="secondary" type="button">Corregir / quitar código</button>':''}<button id="code-scan-another" class="ghost" type="button">Escanear otro</button></div>${allowed.editProduct||allowed.editInventory||allowed.physicalStock?`<details class="code-compact-details code-more-options"><summary>Más opciones</summary><div class="code-action-grid">${allowed.editProduct?'<button id="code-edit-product" class="secondary" type="button">Editar producto</button>':''}${allowed.physicalStock?'<button id="code-found-stock" class="primary" type="button">✓ Encontré stock físico</button>':''}${allowed.editInventory?'<button id="code-edit-stock" class="secondary" type="button">Modificar inventario</button>':''}</div></details>`:''}<div id="code-association-slot"></div></section>`;
}

function associationForm(product){
  return `<section class="code-association-box compact-association-box"><div class="compact-association-head"><div><span class="eyebrow">NUEVO CÓDIGO ASOCIADO</span><h3>Asociar a ${esc(product.code)}</h3></div><small>Código + empaque. La cantidad es obligatoria.</small></div><div class="code-association-grid compact-association-grid"><label class="association-code-field">Código<div class="entrada-con-camara"><input id="associate-code-value" autocomplete="off" placeholder="Escanea o escribe"><button id="associate-code-camera" class="scan-button" type="button" title="Escanear código">▣</button></div></label><label>Presentación<select id="associate-code-presentation"><option value="UNIDAD">Unidad</option><option value="CAJA">Caja</option><option value="PACK">Pack</option><option value="BULTO">Bulto</option><option value="OTRO">Otra</option></select></label><label>Unidades por empaque<input id="associate-code-units" type="number" min="1" step="1" inputmode="numeric" placeholder="Ej. 25" required></label><div class="code-association-actions"><button id="associate-code-save" class="primary" type="button">Asociar</button><button id="associate-code-cancel" class="ghost" type="button">Cancelar</button></div></div><details class="association-optional compact-advanced"><summary>Opciones avanzadas</summary><div class="association-advanced-grid"><label>Origen / canal<select id="associate-code-source"><option value="EASY">Easy</option><option value="SODIMAC">Sodimac</option><option value="CONSTRUMART">Construmart</option><option value="CAJA">Código de caja</option><option value="KAME">Kame</option><option value="SHOPIFY">Shopify / web</option><option value="PROVEEDOR">Proveedor</option><option value="OTRO" selected>Otro</option></select></label><label>En Excel, CANTIDAD significa<select id="associate-code-excel-mode"><option value="UNITS">Unidades finales</option><option value="PACKS">Cajas / packs</option></select></label><label>Etiqueta opcional<input id="associate-code-label" maxlength="80" placeholder="Ej. Caja Easy 25 un."></label></div></details><div id="associate-code-status" class="code-inline-status"></div></section>`;
}

function packagingForm(product,matched){
  const record=matched?.record;if(!record)return '';
  const presentation=String(record.presentation||'UNIDAD').toUpperCase();
  const excelMode=record.excelQtyMode==='PACKS'?'PACKS':'UNITS';
  return `<section class="code-association-box compact-association-box packaging-config-box"><div class="compact-association-head"><div><span class="eyebrow">EMPAQUE DE CÓDIGO EXISTENTE</span><h3>${esc(matched.code)}</h3></div><small>${esc(product.code)} · define cuántas unidades representa este código.</small></div><div class="packaging-config-grid"><label>Presentación<select id="packaging-presentation"><option value="UNIDAD" ${presentation==='UNIDAD'?'selected':''}>Unidad</option><option value="CAJA" ${presentation==='CAJA'?'selected':''}>Caja</option><option value="PACK" ${presentation==='PACK'?'selected':''}>Pack</option><option value="BULTO" ${presentation==='BULTO'?'selected':''}>Bulto</option><option value="OTRO" ${presentation==='OTRO'?'selected':''}>Otra</option></select></label><label>Unidades por empaque<input id="packaging-units" type="number" min="1" step="1" inputmode="numeric" value="${Math.max(1,Number(record.unitsPerPresentation||1))}" required></label><div class="code-association-actions"><button id="packaging-save" class="primary" type="button">Guardar y seguir</button><button id="packaging-cancel" class="ghost" type="button">Cancelar</button></div></div><details class="association-optional compact-advanced"><summary>Ajuste Excel</summary><div class="association-advanced-grid one-column"><label>En Excel, CANTIDAD significa<select id="packaging-excel-mode"><option value="UNITS" ${excelMode==='UNITS'?'selected':''}>Unidades finales</option><option value="PACKS" ${excelMode==='PACKS'?'selected':''}>Cajas / packs</option></select></label></div></details><div id="packaging-status" class="code-inline-status"></div></section>`;
}

function codeManager(product,scanned){
  const records=productCodeRecords(product),clean=normalizeProductCode(scanned);
  return `<section class="code-manager-box"><div class="code-manager-head"><div><span class="eyebrow">CORRECCIÓN RÁPIDA</span><h3>Códigos de ${esc(product.code)}</h3><small>Quita una asociación equivocada sin entrar al editor completo.</small></div><button id="code-manager-close" class="ghost small" type="button">Cerrar</button></div><div class="code-manager-list"><div class="code-manager-row protected ${normalizeProductCode(product.code)===clean?'current':''}"><span><b>${esc(product.code)}</b><small>Código maestro · protegido</small></span><span class="badge">PRINCIPAL</span></div>${records.map(r=>`<div class="code-manager-row ${normalizeProductCode(r.code)===clean?'current':''}"><span><b>${esc(r.code)}</b><small>${esc(r.label||r.source||r.type||'Código asociado')} · ${esc(r.presentation||'UNIDAD')} × ${Math.max(1,Number(r.unitsPerPresentation||1))} · Excel: ${r.excelQtyMode==='PACKS'?'packs':'unidades'}</small></span><button class="ghost small remove-managed-code" data-id="${esc(r.id)}" data-code="${esc(r.code)}" type="button">Quitar</button></div>`).join('')||'<div class="empty-inline"><b>No hay códigos alternativos</b><small>Solo existe el código maestro del producto.</small></div>'}</div></section>`;
}
...
function pickerResults(query){
  ...
  box.querySelectorAll('.code-pick-product').forEach(button=>button.onclick=async()=>{
    ...
    document.querySelector('#existing-product-dialog')?.close();
    const result=document.querySelector('#code-query-result');result.innerHTML=productCard(product,currentCode,permissions());
    document.querySelector('#code-associate-another').onclick=()=>wireAssociation(product);document.querySelector('#code-configure-packaging')?.addEventListener('click',()=>wirePackaging(product,currentCode));document.querySelector('#code-manage-codes').onclick=()=>wireCodeManager(product,currentCode);document.querySelector('#code-scan-another').onclick=()=>resetScan();
    wireAssociation(product);const codeInput=document.querySelector('#associate-code-value');if(codeInput){codeInput.value=currentCode;codeInput.dispatchEvent(new Event('input',{bubbles:true}));}
  });
}
...
function wireAssociation(product){
  ...
  document.querySelector('#associate-code-save').onclick=async()=>{const code=normalizeProductCode(input.value),found=resolveProduct(code),label=document.querySelector('#associate-code-label')?.value||'',source=document.querySelector('#associate-code-source')?.value||'OTRO',presentation=document.querySelector('#associate-code-presentation').value,unitsRaw=document.querySelector('#associate-code-units').value.trim(),units=Number(unitsRaw),excelQtyMode=document.querySelector('#associate-code-excel-mode')?.value||'UNITS';if(!code){toast('Escanea o escribe el código que vas a asociar','warning');return;}if(found){toast(found.id===product.id?'Ese código ya está asociado a este producto':`Ese código pertenece al producto ${found.code}`,'warning');return;}if(!unitsRaw||!Number.isInteger(units)||units<1){document.querySelector('#associate-code-units')?.focus();toast('Indica cuántas unidades contiene este código o empaque','warning');return;}try{await store.commit(state=>addProductCode(state,product.id,code,source,label,{source,presentation,unitsPerPresentation:units,excelQtyMode}),`Código ${code} asociado a ${product.code}`,{operations:['codesAssociate']});await notice('Código asociado',`${code} → ${product.code} · ${presentation} × ${units}. Excel: ${excelQtyMode==='PACKS'?'cantidad en presentaciones':'cantidad en unidades'}.`,'success');showResult(code);}catch(error){toast(error.message||'No fue posible asociar el código','warning');}};
  setTimeout(()=>document.querySelector('#associate-code-camera')?.focus(),0);
}

function wirePackaging(product,scanned){
  if(!permissions().associate){toast('Tu rol no permite configurar empaques','warning');return;}
  const matched=resolveProductCode(scanned),slot=document.querySelector('#code-association-slot');
  if(!matched?.record||matched.product.id!==product.id){toast('Este código no es una asociación editable. Escanea un código asociado al producto.','warning');return;}
  slot.innerHTML=packagingForm(product,matched);
  document.querySelector('#packaging-cancel').onclick=()=>{slot.innerHTML='';document.querySelector('#code-configure-packaging')?.focus();};
  document.querySelector('#packaging-save').onclick=async()=>{
    const unitsInput=document.querySelector('#packaging-units'),unitsRaw=unitsInput.value.trim(),units=Number(unitsRaw),presentation=document.querySelector('#packaging-presentation').value,excelQtyMode=document.querySelector('#packaging-excel-mode')?.value||'UNITS';
    if(!unitsRaw||!Number.isInteger(units)||units<1){unitsInput.focus();toast('La cantidad del empaque es obligatoria y debe ser mayor a 0','warning');return;}
    try{
      await store.commit(state=>{const record=(state.product_codes||[]).find(x=>x.id===matched.record.id);if(!record)throw new Error('Código asociado no encontrado.');record.presentation=presentation;record.unitsPerPresentation=units;record.excelQtyMode=excelQtyMode;},`Empaque ${matched.code}: ${presentation} × ${units}`,{operations:['codesAssociate']});
      toast(`Empaque actualizado: ${matched.code} = ${units} unidades`,'success');
      resetScan();
    }catch(error){toast(error.message||'No fue posible actualizar el empaque','warning');}
  };
  setTimeout(()=>document.querySelector('#packaging-units')?.select(),0);
}
...
function showResult(raw){
  ...
  if(product){
    if(allowed.associate){document.querySelector('#code-associate-another').onclick=()=>wireAssociation(product);document.querySelector('#code-configure-packaging')?.addEventListener('click',()=>wirePackaging(product,code));document.querySelector('#code-manage-codes').onclick=()=>wireCodeManager(product,code);}
