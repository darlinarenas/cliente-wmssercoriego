...
function associationForm(product){
  return `<section class="code-association-box compact-association-box"><div><span class="eyebrow">NUEVO CÓDIGO ASOCIADO</span><h3>Asociar a ${esc(product.code)}</h3><small>El mismo código puede venir de Easy, Sodimac, Construmart, caja, proveedor u otro canal.</small></div><div class="code-association-grid compact-association-grid"><label>Código<div class="entrada-con-camara"><input id="associate-code-value" autocomplete="off" placeholder="Escanea o escribe"><button id="associate-code-camera" class="scan-button" type="button" title="Escanear código">▣</button></div></label><label>Origen / canal<select id="associate-code-source"><option value="EASY">Easy</option><option value="SODIMAC">Sodimac</option><option value="CONSTRUMART">Construmart</option><option value="CAJA">Código de caja</option><option value="KAME">Kame</option><option value="SHOPIFY">Shopify / web</option><option value="PROVEEDOR">Proveedor</option><option value="OTRO" selected>Otro</option></select></label><label>Presentación<select id="associate-code-presentation"><option value="UNIDAD">Unidad</option><option value="CAJA">Caja</option><option value="PACK">Pack</option><option value="BULTO">Bulto</option><option value="OTRO">Otra</option></select></label><label>Unidades por presentación<input id="associate-code-units" type="number" min="1" step="1" value="1" inputmode="numeric"></label><label>En Excel, CANTIDAD significa<select id="associate-code-excel-mode"><option value="UNITS">Unidades finales</option><option value="PACKS">Cajas / packs</option></select><small>Si eliges cajas/packs, Khal multiplica CANTIDAD × unidades por presentación.</small></label><details class="association-optional"><summary>Etiqueta opcional</summary><label>Etiqueta<input id="associate-code-label" maxlength="80" placeholder="Ej. Caja Easy 25 un."></label></details><div class="code-association-actions"><button id="associate-code-save" class="primary" type="button">Asociar ahora</button><button id="associate-code-cancel" class="ghost" type="button">Cancelar</button></div></div><div id="associate-code-status" class="code-inline-status"></div></section>`;
}
...
function pickerResults(query){
  ...
  box.querySelectorAll('.code-pick-product').forEach(button=>button.onclick=async()=>{
    ...
    document.querySelector('#code-associate-another').onclick=()=>wireAssociation(product);document.querySelector('#code-manage-codes').onclick=()=>wireCodeManager(product,currentCode);document.querySelector('#code-scan-another').onclick=()=>resetScan();
