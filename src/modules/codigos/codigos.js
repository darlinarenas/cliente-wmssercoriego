...
function showResult(raw){
  ...
  if(product){
    ...
    if(allowed.editInventory){document.querySelector('#code-found-stock')?.addEventListener('click',()=>openPhysicalStockEntry(product.code,{onSaved:newCode=>showResult(newCode)}));document.querySelector('#code-edit-stock').onclick=()=>openProductEditor(product.code,{onSaved:newCode=>showResult(newCode)});}
