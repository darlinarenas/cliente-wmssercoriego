...
function operatorDashboard(d,user,siteId){
 ...
 const activePicking=orderTasks.filter(o=>['ASIGNADA','EN_PICKING'].includes(o.status)).sort((a,b)=>{const rank=o=>o.status==='EN_PICKING'?2:o.status==='ASIGNADA'?1:0;return rank(b)-rank(a)||String(b.pickingStartedAt||b.assignedAt||b.createdAt||'').localeCompare(String(a.pickingStartedAt||a.assignedAt||a.createdAt||''));});
 ...
 const currentOrder=activePicking[0]||null;
 const orderProgress=currentOrder?(()=>{const items=currentOrder.items||[],totalItems=items.length,doneItems=items.filter(i=>Number(i.pickedQty||0)>=Number(i.qty||0)).length;return {total:totalItems,done:doneItems,pct:totalItems?Math.round(doneItems*100/totalItems):0};})():null;
 const firstName=String(user.name||'Operario').trim().split(/\s+/)[0]||'Operario';
 let nextAction={eyebrow:'TU PRÓXIMA ACCIÓN',title:'Todo al día',detail:'No tienes tareas operativas pendientes en este momento.',href:'#/ordenes',button:'Ver mis tareas',pct:null};
 if(currentOrder){nextAction={eyebrow:'TU PRÓXIMA ACCIÓN',title:currentOrder.status==='EN_PICKING'?'Continuar picking':'Iniciar picking',detail:`Orden ${currentOrder.externalNumber||currentOrder.id} · ${orderProgress.done}/${orderProgress.total} productos`,href:`#/ordenes?openOrder=${encodeURIComponent(currentOrder.id)}`,button:currentOrder.status==='EN_PICKING'?'Seguir trabajando':'Comenzar orden',pct:orderProgress.pct};}
 else if(inventoryTasks.length){nextAction={eyebrow:'TU PRÓXIMA ACCIÓN',title:'Continuar inventario',detail:`${inventoryTasks.length} conteo(s) asignado(s) en este centro`,href:'#/inventarios',button:'Abrir inventario',pct:null};}
 else if(putaway.length){nextAction={eyebrow:'TU PRÓXIMA ACCIÓN',title:'Ubicar producto',detail:`${putaway.length} tarea(s) de ubicación pendiente(s)`,href:'#/tareas-ubicacion',button:'Continuar ubicación',pct:null};}
 const actions=[
 ...
 return `<section class="operator-compact-home"><a class="operator-next-action compact ${total?'has-work':'is-clear'}" href="${nextAction.href}"><div><span class="eyebrow">QUÉ HACER AHORA</span><h3>${esc(nextAction.title)}</h3><p>${esc(nextAction.detail)}</p>${nextAction.pct!==null?`<div class="operator-next-progress"><div><span style="width:${nextAction.pct}%"></span></div><b>${nextAction.pct}%</b></div>`:''}</div><strong>${esc(nextAction.button)} <span>→</span></strong></a></section>${inventoryTasks.length?`<a class="operator-inventory-alert compact" href="#/inventarios"><span>▦</span><div><small>INVENTARIO ASIGNADO</small><b>${inventoryTasks.length} conteo(s) pendiente(s)</b></div><strong>→</strong></a>`:''}<div class="operator-action-grid operator-action-grid-compact">${actions.map(([id,icon,label,help])=>`<a href="#/${id}" class="operator-action-card ${id==='ordenes'&&total?'has-tasks':''}"><span>${icon}</span><div><b>${label}</b><small>${id==='ordenes'?`${total} pendiente(s) · ${reviewOrders.length} en revisión`:help}</small></div>${id==='ordenes'&&total?`<em>${total}</em>`:''}</a>`).join('')}</div>`;
}
...
function managerOperationalDashboard(d,user,siteId,effectiveRole){
 ...
 const loadsNeedDriver=shipments.filter(s=>s.status==='LISTA_RETIRO'&&!s.transporterUserId&&!s.externalTransporter?.name),loadsAwaitCustody=shipments.filter(s=>s.status==='LISTA_RETIRO'&&Boolean(s.transporterUserId||s.externalTransporter?.name)),loadsInTransit=shipments.filter(s=>s.status==='EN_TRANSITO');
 ...
 const recentActivity=[...(d.audit||[])].filter(a=>a.at&&(!a.siteId||allowedIds.has(a.siteId))).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,5);
 ...
 return `<div class="manager-lite-dashboard">
  ...
  <section class="manager-lite-card"><div class="manager-lite-card-head"><div><span class="eyebrow">EN VIVO</span><h3>Actividad reciente</h3></div><a href="#/historial">Ver todo</a></div><div class="manager-lite-orders">${recentActivity.length?recentActivity.map(a=>`<a href="#/historial"><span><b>${esc(a.message)}</b><small>${esc(userName(d,a.userId))} · ${fmtDate(a.at)}</small></span><em>◷</em></a>`).join(''):empty('Sin actividad reciente','Las últimas acciones del equipo aparecerán aquí.')}</div></section></div>
...
function monitorInventoryQuickView(d,s){
 ...
 return `<section class="exec-inventory-preview"><div class="exec-inventory-preview-head"><div><small class="exec-preview-eyebrow">SEGUIMIENTO DEL INVENTARIO</small><h4>${esc(s.number||s.id)}</h4><p>${esc(s.name||'Inventario')}</p><small>${esc((d.sites||[]).find(x=>x.id===s.siteId)?.name||s.siteId)}</small></div>${badge(status,s.status==='EN_REVISION'?'warn':s.status==='ANULADA'?'bad':'ok')}</div><div class="exec-preview-hero"><div class="exec-preview-ring" style="--progress:${progress.percent}%" aria-hidden="true"><strong>${progress.percent}%</strong></div><div><b>Avance del conteo</b><p>${progress.covered} de ${progress.target} posiciones cubiertas</p><small>Estimado por posiciones. Los racks terminados cuentan como revisados.</small></div></div><div class="exec-preview-track" role="progressbar" aria-label="Avance del inventario" aria-valuenow="${progress.percent}" aria-valuemin="0" aria-valuemax="100"><i style="width:${progress.percent}%"></i></div><div class="exec-preview-metrics"><div><small>Racks terminados</small><b>${done}<em> / ${racks}</em></b></div><div><small>Posiciones cubiertas</small><b>${progress.covered}<em> / ${progress.target}</em></b></div><div><small>Responsables enviados</small><b>${sent}<em> / ${assignments.length}</em></b></div><div><small>Líneas contadas</small><b>${(s.lines||[]).length}</b></div></div>${found?`<div class="exec-preview-found"><b>${found} código(s) encontrados fuera del catálogo</b><small>Consulta el detalle en el inventario completo.</small></div>`:''}${assignments.length?`<details class="exec-preview-team"><summary>Ver avance por responsable <small>${assignments.length} personas</small></summary><div class="exec-preview-team-list">${assignments.map(a=>{const p=monitorInventoryProgress(d,{...s,assignments:[a]}),name=userName(d,a.userId),initials=String(name).trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('');return `<div class="exec-preview-person"><div class="exec-preview-avatar" aria-hidden="true">${esc(initials)}</div><div class="exec-preview-person-info"><b>${esc(name)}</b><small>${a.status==='ENVIADO_REVISION'?'Enviado a revisión':a.status==='EN_CONTEO'?'En conteo':'Pendiente'}</small><div class="exec-preview-person-track" aria-hidden="true"><i style="width:${p.percent}%"></i></div></div><strong>${p.percent}%</strong></div>`;}).join('')}</div></details>`:''}<div class="exec-preview-open">${monitorRecordLink('Abrir inventario completo','Conteos, posiciones y revisión','inventarios',s.siteId,s.id)}</div></section>`;
}
...
function wireExecutiveMonitor(){
 ...
 document.querySelectorAll('[data-monitor]').forEach(button=>button.addEventListener('click',event=>{
  ...
  else if(key==='dispatch-assignment'){label='Despachos por asignar';rows=shipments.filter(x=>x.status==='LISTA_RETIRO'&&!x.transporterUserId&&!x.externalTransporter?.name).map(load);}
  ...
  else if(key==='alerts'){label='Alertas operativas';rows=[...orders.filter(o=>o.status==='ESPERANDO_REPOSICION').map(order),...shipments.filter(x=>String(x.status).includes('INCID')||(x.status==='LISTA_RETIRO'&&!x.transporterUserId&&!x.externalTransporter?.name)).map(load)];}
...
function adminExecutiveDashboard(d,user,siteId){
 ...
 const dispatchNeedsAssignment=shipments.filter(s=>s.status==='LISTA_RETIRO'&&!s.transporterUserId&&!s.externalTransporter?.name).length;
 const pendingAdmin=archivedInv.length+archivedOrders.length+reviewInv.length;
 const alerts=orders.filter(o=>['ESPERANDO_REPOSICION'].includes(o.status)).length+shipments.filter(s=>String(s.status).includes('INCID')).length+dispatchNeedsAssignment;
 ...
 const audit=[...(d.audit||[])].filter(a=>a.at&&(!a.siteId||siteIds.has(a.siteId))).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,5);
 ...
 const pendingItems=[
  ...
  ['Despachos por asignar','Orden emitida · asigna conductor o retiro',dispatchNeedsAssignment,'#/cargas'],
...
export function renderDashboard(root){
 ...
 if(isExecutiveAdmin){root.innerHTML=shell('Panel Ejecutivo',adminExecutiveDashboard(d,sessionUser,siteId),'dashboard');wireShell();wireExecutiveMonitor();hydrateEconomicIndicators();startSilentRefresh('dashboard','#/dashboard',()=>renderDashboard(root),{interval:8000});return;}
 if(['ENCARGADO','ADMINISTRADOR'].includes(effectiveRole)){root.innerHTML=shell('Inicio',managerOperationalDashboard(d,sessionUser,siteId,effectiveRole),'dashboard');wireShell();startSilentRefresh('dashboard','#/dashboard',()=>renderDashboard(root),{interval:5000,collections:['orders','shipments','tasks','audit','movements','receipts','transfers','pallets']});return;}
