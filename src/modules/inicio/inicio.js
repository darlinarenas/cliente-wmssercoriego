...
function monitorInventoryChart(sessions){const days=monitorInventoryDays(sessions),max=Math.max(1,...days.map(d=>d.count)),total=days.reduce((n,d)=>n+d.count,0);return `<section class="exec-card exec-inventory-chart"><div class="exec-card-head"><div><h3>Comportamiento del inventario</h3><small>Líneas de conteo registradas por día · últimos 7 días · empresa activa</small></div><button class="ghost" data-monitor="inventories" type="button">Ver inventarios</button></div><div class="exec-chart-bars" role="img" aria-label="${esc(days.map(d=>`${d.label}: ${d.count} líneas`).join('; '))}">${days.map(d=>`<div class="exec-chart-day"><b>${d.count}</b><div><i style="height:${d.count/max*100}%"></i></div><small>${esc(d.label)}</small></div>`).join('')}</div><p>${total} línea(s) registradas. Una línea corresponde a un registro de conteo, no a unidades ni posiciones únicas.</p>${total?'':'<small>Sin conteos registrados en este período.</small>'}</section>`;}
...
function wireExecutiveMonitor(){
 ...
 document.querySelectorAll('[data-monitor]').forEach(button=>button.addEventListener('click',event=>{
  ...
  else if(key==='inventories'){label='Inventarios de la empresa';rows=sessions.map(inv);}
...
function adminExecutiveDashboard(d,user,siteId){
 ...
 return `<div class="exec-dashboard">
  ...
  ${monitorInventoryChart(sessions)}
