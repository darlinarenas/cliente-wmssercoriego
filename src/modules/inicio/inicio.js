...
function operatorDashboard(d,user,siteId){
 ...
 return `<section class="operator-premium-home"><div class="operator-premium-heading"><div><span class="eyebrow">PANEL DEL OPERARIO</span><h2>Hola, ${esc(firstName)} 👋</h2><p>Tu trabajo de hoy, ordenado por prioridad.</p></div><div class="operator-live-dot"><i></i> Actualización silenciosa activa</div></div><a class="operator-next-action ${total?'has-work':'is-clear'}" href="${nextAction.href}"><div><span class="eyebrow">${nextAction.eyebrow}</span><h3>${esc(nextAction.title)}</h3><p>${esc(nextAction.detail)}</p>${nextAction.pct!==null?`<div class="operator-next-progress"><div><span style="width:${nextAction.pct}%"></span></div><b>${nextAction.pct}%</b></div>`:''}</div><strong>${esc(nextAction.button)} <span>→</span></strong></a><div class="operator-today-metrics"><a href="#/ordenes"><small>ÓRDENES</small><b>${orderTasks.length}</b><span>pendiente(s)</span></a><a href="#/inventarios"><small>INVENTARIO</small><b>${inventoryTasks.length}</b><span>tarea(s)</span></a><a href="#/tareas-ubicacion"><small>UBICACIONES</small><b>${putaway.length}</b><span>por organizar</span></a><div><small>EN REVISIÓN</small><b>${reviewOrders.length+inventoryReview.length}</b><span>enviado(s)</span></div></div></section>${inventoryTasks.length?`<a class="operator-inventory-alert" href="#/inventarios"><span>▦</span><div><small>TAREA DE INVENTARIO</small><b>Te asignaron un conteo de inventario</b><em>${inventoryTasks.map(t=>(t.assignment.rackIds||[]).join(', ')).join(' · ')}</em></div><strong>${inventoryTasks.length}</strong></a>`:''}<section class="operator-other-actions"><div class="operator-other-actions-head"><span class="eyebrow">OTRAS ACCIONES</span><h3>Herramientas de operación</h3></div><div class="operator-action-grid">${actions.map(([id,icon,label,help])=>`<a href="#/${id}" class="operator-action-card ${id==='ordenes'&&total?'has-tasks':''}"><span>${icon}</span><div><b>${label}</b><small>${id==='ordenes'?`${total} pendiente(s) · ${reviewOrders.length} en revisión`:help}</small></div>${id==='ordenes'&&total?`<em>${total}</em>`:''}</a>`).join('')}</div></section>${putaway.length?`<section class="panel operator-pending-panel"><div class="panel-head"><div><span class="eyebrow">TAREAS DE UBICACIÓN</span><h3>${putaway.length} asignada(s) a tu usuario</h3></div><a class="primary" href="#/tareas-ubicacion">Ver mis tareas</a></div></section>`:''}`;
}
