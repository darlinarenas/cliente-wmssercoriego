...
function managerOperationalDashboard(d,user,siteId,effectiveRole){
 ...
 const recentActivity=[...(d.audit||[])].filter(a=>a.at&&(!a.siteId||allowedIds.has(a.siteId))).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,8);
...
export function renderDashboard(root){
 ...
 if(['ENCARGADO','ADMINISTRADOR'].includes(effectiveRole)){root.innerHTML=shell('Inicio',managerOperationalDashboard(d,sessionUser,siteId,effectiveRole),'dashboard');wireShell();startSilentRefresh('dashboard','#/dashboard',()=>renderDashboard(root),{collections:['orders','tasks','audit','movements','receipts','transfers','pallets']});return;}
