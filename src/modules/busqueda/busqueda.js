...
import { stockBySite,activeSiteId,stockSitesOrdered,inventorySiteId } from '../../services/stock.js';
...
function results(q){
  ...
  return matches.map(p=>{
    const active=activeSiteId(d),site=d.sites.find(s=>s.id===active),allInv=d.inventory.filter(i=>i.productCode===p.code&&i.qty>0),inv=allInv.filter(i=>inventorySiteId(i,d)===active),total=inv.reduce((a,b)=>a+b.qty,0),siteRows=stockSitesOrdered(p.code,d);
    const inventoryBySite=d.sites.map(s=>({site:s,rows:allInv.filter(i=>inventorySiteId(i,d)===s.id)})).filter(x=>x.rows.length);
