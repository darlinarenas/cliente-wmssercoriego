...
function results(q){
  ...
  return matches.map(p=>{
    const active=activeSiteId(d),site=d.sites.find(s=>s.id===active),inv=d.inventory.filter(i=>i.productCode===p.code&&i.qty>0&&inventorySiteId(i,d)===active),total=inv.reduce((a,b)=>a+b.qty,0),siteRows=stockSitesOrdered(p.code,d);
