...
import { stockBySite,activeSiteId,stockSitesOrdered,inventorySiteId,totalCompanyStock } from '../../services/stock.js';
...
function results(q){
  ...
  return matches.map(p=>{
    ...
    const ubicaciones=inventoryBySite.length?`<div class="search-location-groups">${inventoryBySite.map(group=>`<section class="search-location-group ${group.site.id===active?'active':''}"><div class="search-location-group-head"><div><small>${group.site.id===active?'Centro activo':'Otra sucursal'}</small><b>${esc(group.site.name)}</b></div><strong>${group.rows.reduce((a,b)=>a+Number(b.qty||0),0)} un.</strong></div><div class="location-list">${group.rows.map(i=>{const loc=d.locations.find(l=>l.id===i.locationId);const label=loc?vistaCodigoUbicacion(loc,d):(i.locationId||'Sin ubicación');return `<div><div><b>${esc(label)}</b><small>${i.palletId?`Palet ${esc(i.palletId)}`:'Ubicación directa'} · ${esc(i.locationId||'Sin código')}</small></div><strong>${i.qty} un.</strong></div>`;}).join('')}</div></section>`).join('')}</div>`:`<p class="muted">Sin stock localizado en ningún centro.</p>`;
    return `<article class="product-result"><div class="product-title"><div><span class="sku">Código ${esc(p.code)}</span><h3>${esc(p.name)}</h3><small><b>Descripción:</b> ${esc(p.description||'No registrada')} · <b>Tipo:</b> ${esc(p.type||p.family||'Sin clasificar')} · <b>Códigos:</b> ${esc(productAliases(p).join(', '))}</small></div><div class="product-stock-head"><div class="cantidad-destacada global-wms-stock"><small>STOCK GLOBAL WMS · EMPRESA ACTIVA</small><strong>${globalTotal} un.</strong></div><div class="cantidad-destacada active-site-stock"><small>${esc(site?.name||active)} · CENTRO ACTIVO</small>${badge(`${total} un.`,total?'ok':'warn')}${transitTotal?`<small>En tránsito: <b>${transitTotal} un.</b></small>`:''}</div></div></div><div class="search-site-stocks">${siteRows.map(x=>`<span class="${x.active?'active':''}"><small>${x.active?'Centro activo':'Otra sucursal'}</small><b>${esc(x.name)}</b><strong>${x.qty} un.</strong></span>`).join('')}</div><div class="search-location-title"><b>Ubicación física por centro</b><small>Consulta únicamente. Las ubicaciones de otras sucursales no se editan desde aquí.</small></div>${ubicaciones}${transit.length?`<div class="transit-list"><b>Unidades fuera de bodega / EN TRÁNSITO</b>${transit.map(t=>`<div><span><b>${esc(t.id)}</b> → ${esc(t.destination||'Destino')}</span><small>Retira / conduce: ${esc(t.driver||'No registrado')} · ${t.departedAt?new Date(t.departedAt).toLocaleString('es-CL'):'Sin hora'}</small><strong>${t.qty} un.</strong></div>`).join('')}</div>`:''}<div class="result-actions"><a href="#/mapa3d?code=${encodeURIComponent(p.code)}" class="primary">Ver en mapa 3D</a><a href="#/movimientos?code=${encodeURIComponent(p.code)}" class="secondary">Mover / reubicar</a><a href="#/transferencias?code=${encodeURIComponent(p.code)}" class="ghost">Preparar despacho</a><button class="ghost edit-product" data-code="${esc(p.code)}">Editar / Inventario</button><button class="ghost copy-code" data-code="${esc(p.code)}">Copiar código</button></div></article>`;
  }).join('');
}
