...
function results(q){
  ...
  return matches.map(p=>{
    ...
    return `<article class="product-result"><div class="product-title"><div><span class="sku">Código ${esc(p.code)}</span><h3>${esc(p.name)}</h3><small><b>Descripción:</b> ${esc(p.description||'No registrada')} · <b>Tipo:</b> ${esc(p.type||p.family||'Sin clasificar')}</small></div><div class="cantidad-destacada"><small>Cantidad total</small>${badge(`${total} un.`,total?'ok':'warn')}</div></div><div class="location-list">${inv.length?inv.map(i=>`<div><div><b>${esc(i.locationId)}</b><small>${i.palletId?`Palet ${esc(i.palletId)}`:'Ubicación directa'}</small></div><strong>${i.qty} un.</strong></div>`).join(''):'<p class="muted">Producto conocido, pero aún sin ubicación registrada.</p>'}</div><div class="result-actions"><a href="#/movimientos?code=${encodeURIComponent(p.code)}" class="secondary">Mover / reubicar</a><a href="#/transferencias?code=${encodeURIComponent(p.code)}" class="ghost">Preparar despacho</a><button class="ghost edit-product" data-code="${esc(p.code)}">Editar / Inventario</button><button class="ghost copy-code" data-code="${esc(p.code)}">Copiar código</button></div></article>`;
  }).join('');
}
