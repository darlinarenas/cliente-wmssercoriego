...
function results(q){
  ...
  return matches.map(p=>{
    ...
    const transit=d.transfers.filter(t=>t.status==='EN_TRANSITO'&&!t.externalTransporter?.name&&(t.items||[]).some(x=>x.code===p.code)).map(t=>({id:t.id,qty:(t.items||[]).filter(x=>x.code===p.code).reduce((a,b)=>a+Number(b.qty||0),0),destination:t.destinationName,driver:t.driver,departedAt:t.departedAt}));
