...
function filaProducto(p){
  ...
  return `<tr><td><b>${esc(p.code)}</b></td><td><b>${esc(p.name)}</b><small class="table-description">${esc(p.description||'Descripción no registrada')}</small></td><td>${esc(p.family)}</td><td>${badge(p.rotation,p.rotation==='ALTA'?'danger':p.rotation==='MEDIA'?'warn':'neutral')}</td><td><b>${total}</b></td><td>${p.pickingLocationId?esc(p.pickingLocationId):'<span class="muted">Sin ubicación de preparación rápida</span>'}</td><td><button class="ghost small edit-product-row" data-code="${esc(p.code)}">Editar / Inventario</button></td></tr>`;
}
...
function pintarTabla(){
  ...
  cuerpo.innerHTML=lista.length?lista.map(filaProducto).join(''):`<tr><td colspan="7">${empty('Sin coincidencias','Cambia los filtros o la palabra de búsqueda.')}</td></tr>`;
...
export function renderProducts(root){
  ...
  <div class="table-wrap"><table><thead><tr><th>Código</th><th>Descripción</th><th>Tipo / familia</th><th>Rotación</th><th>Cantidad localizada</th><th>Preparación rápida (Picking)</th><th>Acciones</th></tr></thead><tbody id="cuerpo-productos">${d.products.map(filaProducto).join('')}</tbody></table></div>`,'productos');
  wireShell();
