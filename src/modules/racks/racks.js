...
function openPositionDialog(locationId){
 ...
 dlg.innerHTML=`<div class="rack-position-card"><div class="dialog-head"><div><span class="eyebrow">POSICIÓN / REPISA</span><h3>${esc(locationId)}</h3><small>${esc(loc.rackId||'Rack')} · ${rows.length} producto${rows.length===1?'':'s'} · ${total} unidades</small></div><button type="button" id="close-rack-position" class="ghost">×</button></div>
   ...
   <div class="dialog-actions">${canInitialStockLoad()?'<button type="button" id="rack-position-initial-stock" class="primary">+ Iniciar carga desde cero</button>':''}<button type="button" id="close-rack-position-bottom" class="ghost">Cerrar</button></div></div>`;
 dlg.querySelector('#close-rack-position').onclick=()=>dlg.close();
 dlg.querySelector('#close-rack-position-bottom').onclick=()=>dlg.close();
 dlg.querySelector('#rack-position-initial-stock')?.addEventListener('click',()=>{dlg.close();openInitialStockSession({locationId,title:`${locationId} · carga desde cero`,onFinished:()=>{const active=document.querySelector('#rack-detail-title');if(active)pintarDetalle(loc.rackId);}});});
 dlg.showModal();
}
...
export function renderRacks(root){
 ...
 root.innerHTML=shell('Racks',`<div class="page-intro"><div><span class="eyebrow">MAPA LÓGICO · CENTRO ACTIVO</span><h2>${esc(site?.name||siteId)}</h2><p>Solo se muestran los racks y ubicaciones físicas del centro activo. Para administrar otra bodega, cambia primero el centro activo.</p></div>${canInitialStockLoad()?'<button id="open-position-initial-stock" class="primary" type="button">+ Cargar posición desde cero</button>':''}</div><div class="rack-grid">${cards||empty('Sin racks configurados',`Este centro todavía no tiene racks. Créalo desde Estructura para ${esc(site?.name||siteId)}.`)}</div><div id="rack-detail"></div>${canInitialStockLoad()?`<dialog id="position-initial-stock-picker" class="rack-position-dialog"><div class="rack-position-card"><div class="dialog-head"><div><span class="eyebrow">STOCK INICIAL</span><h3>Selecciona una posición</h3><small>Escanea o elige la posición física que vas a llenar desde cero.</small></div><button id="close-position-initial-stock-picker" class="ghost" type="button">×</button></div><label>Escanear posición<div class="entrada-con-camara"><input id="position-initial-stock-code" placeholder="Escanea o escribe REC-R#-M#-N#" autocomplete="off"><button id="position-initial-stock-scan" class="scan-button" type="button" title="Escanear posición">▣</button></div></label><label>O seleccionar posición<select id="position-initial-stock-select"><option value="">Seleccionar posición…</option>${d.locations.filter(l=>l.siteId===siteId&&l.active&&l.rackId).sort((a,b)=>String(a.id).localeCompare(String(b.id),undefined,{numeric:true})).map(l=>`<option value="${esc(l.id)}">${esc(l.label||l.id)} · ${esc(l.id)}</option>`).join('')}</select></label><details class="rack-initial-map-help"><summary>▦ Ayuda de mapa</summary><div class="rack-content-map-head"><div><span class="eyebrow">PASO 1 · RACK</span><h4>Selecciona el rack físico</h4><small>Primero toca el rack; después Khal te muestra solamente sus posiciones.</small></div></div><div class="rack-grid initial-stock-rack-map">${racks.map(r=>`<button type="button" class="rack-card initial-stock-map-rack" data-rack="${esc(r.id)}"><div class="rack-visual"><span>${esc(r.rackCode||r.name||r.id)}</span><div>${r.modules?`${r.modules} módulos × ${r.levels} niveles`:'Rack'}</div></div><div class="rack-card-body"><b>${esc(r.name||r.id)}</b><small>Tocar para ver posiciones</small></div></button>`).join('')}</div><div id="initial-stock-map-positions" class="rack-content-map" hidden></div></details><small id="position-initial-stock-status">Escanea la etiqueta de la posición, selecciónala o usa el mapa.</small><div class="dialog-actions"><button id="cancel-position-initial-stock-picker" class="ghost" type="button">Cancelar</button><button id="start-position-initial-stock" class="primary" type="button">Iniciar carga desde cero</button></div></div></dialog>`:''}`,'racks'); wireShell();
 document.querySelectorAll('.rack-card-click').forEach(c=>c.onclick=()=>pintarDetalle(c.dataset.rack));
 const picker=document.querySelector('#position-initial-stock-picker'),positionCode=document.querySelector('#position-initial-stock-code'),positionSelect=document.querySelector('#position-initial-stock-select'),positionStatus=document.querySelector('#position-initial-stock-status');const resolveInitialPosition=()=>{const typed=positionCode?.value?.trim(),scanned=typed?buscarUbicacionPorCodigo(typed,store.data):null,locationId=scanned?.siteId===siteId&&scanned?.rackId?scanned.id:positionSelect?.value||'';if(positionStatus)positionStatus.textContent=locationId?`✓ Posición lista: ${locationId}`:(typed?'Código de posición no reconocido en este centro.':'Escanea la etiqueta de la posición, selecciónala o usa el mapa.');return locationId;};document.querySelector('#open-position-initial-stock')?.addEventListener('click',()=>picker?.showModal());document.querySelector('#close-position-initial-stock-picker')?.addEventListener('click',()=>picker?.close());document.querySelector('#cancel-position-initial-stock-picker')?.addEventListener('click',()=>picker?.close());if(positionCode)positionCode.oninput=()=>{if(positionCode.value&&positionSelect)positionSelect.value='';resolveInitialPosition();};if(positionSelect)positionSelect.onchange=()=>{if(positionSelect.value&&positionCode)positionCode.value='';resolveInitialPosition();};enlazarBotonEscaner('position-initial-stock-scan','position-initial-stock-code',{titulo:'Escanear posición',ayuda:'Apunta a la etiqueta del rack / posición',onDetectar:()=>resolveInitialPosition()});document.querySelectorAll('.initial-stock-map-rack').forEach(b=>b.onclick=()=>{const rackId=b.dataset.rack,r=d.racks.find(x=>x.id===rackId&&x.siteId===siteId),box=document.querySelector('#initial-stock-map-positions');if(!r||!box)return;const locs=d.locations.filter(l=>l.siteId===siteId&&l.active&&l.rackId===rackId).sort((a,b)=>String(a.id).localeCompare(String(b.id),undefined,{numeric:true}));box.hidden=false;box.innerHTML=`<div class="rack-content-map-head"><div><span class="eyebrow">PASO 2 · POSICIÓN</span><h4>${esc(r.name||r.id)}</h4><small>Toca la posición física que vas a cargar.</small></div></div><div class="rack-position-list">${locs.map(l=>`<button type="button" class="position-chip initial-stock-map-position" data-location="${esc(l.id)}"><b>${esc(l.label||l.id)}</b><small>${esc(l.id)}</small></button>`).join('')}</div>`;box.querySelectorAll('.initial-stock-map-position').forEach(pos=>pos.onclick=()=>{if(positionCode)positionCode.value='';if(positionSelect)positionSelect.value=pos.dataset.location;resolveInitialPosition();});box.scrollIntoView({behavior:'smooth',block:'nearest'});});document.querySelector('#start-position-initial-stock')?.addEventListener('click',()=>{const locationId=resolveInitialPosition();if(!locationId){positionCode?.focus();return;}picker?.close();openInitialStockSession({locationId,title:`${locationId} · carga desde cero`,onFinished:()=>renderRacks(root)});});
}
