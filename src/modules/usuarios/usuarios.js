...
<dialog id="user-dialog"><form id="user-form" class="dialog-card">
  ...
  <label>Rol<select id="user-role"><option value="OPERADOR_BODEGA">Operador de bodega</option><option value="OPERADOR_RECEPCION">Operador de recepción</option><option value="ENCARGADO">Encargado</option><option value="ADMINISTRADOR">Administrador</option></select></label>
...
export function renderUsers(root){
  ...
  <section class="panel"><div class="panel-head"><div><h3>Personas registradas</h3><small>Solo usuarios activos pueden iniciar sesión y aparecen en listas de responsables.</small></div><span>${store.data.users.length} usuarios</span></div><div class="user-admin-list">${rows()}</div></section>${dialogs()}`;
...
function wireUsers(root){
  ...
  const open=(u=null)=>{
    ...
    document.querySelector('#user-role').value=u?.role||'OPERADOR_BODEGA';
    const companies=document.querySelector('#user-companies');
    companies.innerHTML=(store.data.companies||[]).map(c=>`<option value="${esc(c.id)}">${esc(c.name)}</option>`).join('');
    ...
    const renderSites=()=>{
      ...
      sites.innerHTML=store.data.sites.filter(s=>!selectedCompanies.length||selectedCompanies.includes(siteCompanyId(s))).map(s=>`<option value="${esc(s.id)}">${esc(companyName(siteCompanyId(s)))} · ${esc(s.name)}</option>`).join('');
