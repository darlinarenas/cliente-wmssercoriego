...
export function chooseCompany(companies,onChoose,{authenticated=false,onBack=null}={}){
 ...
 document.querySelector('#app').innerHTML=`<main class="login-page"><section class="login-card company-entry-card"><div class="login-brand"><div class="brand-mark">K</div><div><b>Khal</b><small>Vexhora Group</small></div></div><div><span class="eyebrow">EMPRESA DE INGRESO</span><h1>¿Dónde quieres entrar?</h1><p>Primero elige la empresa. Dentro de ella podrás seleccionar el centro, tienda o sucursal.</p></div><div class="company-entry-list">${active.map(c=>`<button class="company-entry-option" data-company="${esc(c.id)}" type="button"><span>${esc((c.name||c.id).slice(0,1).toUpperCase())}</span><div><b>${esc(c.name||c.id)}</b><small>${esc(c.notes||'Abrir entorno de trabajo')}</small></div><strong>Entrar →</strong></button>`).join('')}</div><button id="company-entry-back" class="ghost" type="button">${authenticated?'Volver a Khal':'Cerrar sesión'}</button></section></main>`;
...
export function renderLogin(root,onSuccess){
 ...
 root.innerHTML=`<main class="login-page"><section class="login-card"><div class="login-brand"><div class="brand-mark">K</div><div><b>Khal</b><small>Vexhora Group</small></div></div><div><span class="eyebrow">ACCESO SEGURO</span><h1>Iniciar sesión</h1><p>Ingresa con el usuario y contraseña asignados por el administrador.</p></div><form id="login-form"><label>Usuario<input id="login-user" autocomplete="username" required autofocus></label><label>Contraseña<input id="login-password" type="password" autocomplete="current-password" required></label><div id="login-error" class="login-error" hidden></div><button class="primary login-submit" type="submit">Entrar a Khal</button></form><small class="login-help">El acceso se valida directamente contra los usuarios registrados en la base de datos y carga el rol correspondiente.</small></section></main>`;
