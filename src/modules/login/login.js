...
function chooseCompany(companies,onChoose){
 const active=companies.filter(c=>c.active!==false);
 document.querySelector('#app').innerHTML=`<main class="login-page"><section class="login-card company-entry-card"><div class="login-brand"><div class="brand-mark">S</div><div><b>WMS Multiempresa</b><small>Selecciona el entorno de trabajo</small></div></div><div><span class="eyebrow">EMPRESA DE INGRESO</span><h1>¿Dónde quieres entrar?</h1><p>Primero elige la empresa. Dentro de ella podrás seleccionar el centro, tienda o sucursal.</p></div><div class="company-entry-list">${active.map(c=>`<button class="company-entry-option" data-company="${esc(c.id)}" type="button"><span>${esc((c.name||c.id).slice(0,1).toUpperCase())}</span><div><b>${esc(c.name||c.id)}</b><small>${esc(c.notes||'Abrir entorno de trabajo')}</small></div><strong>Entrar →</strong></button>`).join('')}</div><button id="company-entry-back" class="ghost" type="button">Cerrar sesión</button></section></main>`;
 document.querySelectorAll('.company-entry-option').forEach(b=>b.onclick=()=>onChoose(b.dataset.company));
 document.querySelector('#company-entry-back').onclick=()=>{auth.logout();location.reload();};
}
