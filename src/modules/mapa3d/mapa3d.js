...
export function renderMap3d(root){
 ...
 const rotateBtn=document.querySelector('#map3d-rotate');if(rotateBtn)rotateBtn.onclick=()=>{if(!edit||!selectedKey)return;const slot=layout[selectedKey];if(!slot)return;const centerX=Number(slot.x||0)+Number(slot.w||0)/2,centerY=Number(slot.y||0)+Number(slot.h||0)/2,nextW=Number(slot.h||0),nextH=Number(slot.w||0);slot.w=nextW;slot.h=nextH;slot.x=Math.max(0,Math.min(100-nextW,centerX-nextW/2));slot.y=Math.max(0,Math.min(100-nextH,centerY-nextH/2));paint();toast(`${selectedKey} rotado a orientación ${slot.h>=slot.w?'vertical':'horizontal'}. Guarda el mapa para conservar el cambio.`,'info');};
 const editBtn=document.querySelector('#map3d-edit'); if(editBtn)editBtn.onclick=async()=>{if(!edit){edit=true;editBtn.textContent='Guardar mapa';editBtn.classList.remove('secondary');editBtn.classList.add('primary');toast('Modo edición: arrastra los racks para moverlos. Selecciona uno y usa “Rotar rack” para cambiar vertical/horizontal.','info');paint();return;}try{await store.commit(d=>{d.settings=d.settings||{};d.settings.map3dLayouts=d.settings.map3dLayouts||{};d.settings.map3dLayouts[siteId]=layout;},`Mapa 3D actualizado · ${site?.name||siteId}`);edit=false;editBtn.textContent='Editar mapa';editBtn.classList.add('secondary');editBtn.classList.remove('primary');paint();await notice('Mapa guardado','La distribución física del centro quedó guardada. La estructura de módulos y niveles continúa administrándose desde Estructura.','success');}catch(e){await notice('No se pudo guardar',e.message||'Error al guardar el mapa.','error');}};
