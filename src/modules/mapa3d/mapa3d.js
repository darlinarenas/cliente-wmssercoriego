...
export function renderMap3d(root){
 ...
 root.innerHTML=shell('Mapa 3D de bodega',`<div class="page-intro map3d-intro"><div><span class="eyebrow">MAPA OPERATIVO · CENTRO ACTIVO</span><h2>${esc(site?.name||siteId)}</h2><p>Busca un producto para ver visualmente en qué rack está. El mapa usa el inventario y las ubicaciones reales del WMS.</p></div><div class="map3d-top-actions"><button id="map3d-edit" class="secondary" type="button" ${isAdmin()?'':'hidden'}>Editar mapa</button></div></div>
