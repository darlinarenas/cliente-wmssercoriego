...
const nav=[
 ...
 ['codigos-cantidades','Códigos y cantidades','×'],
...
export function shell(title,content,active='dashboard'){
  ...
  const effectiveRole=(currentUser?.accessAssignments||[]).find(a=>a.siteId===activeSite)?.role||currentUser?.role;
  const operatorMenu=new Set(['dashboard','buscar','codigos','codigos-cantidades','ordenes','recepciones','organizar-recibidos','recepcion-traspasos','tareas-ubicacion','transferencias','cargas','palets','movimientos','inventarios','etiquetas','etiquetas-productos','etiquetas-racks','etiquetas-pallets','mapa3d']);
